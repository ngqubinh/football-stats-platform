...
public class CrawlingService : ICrawlingService
{
        ...
        ILogger<CrawlingService> logger)
    {
        ...
        this._httpClient.Timeout = TimeSpan.FromSeconds(120);
    }
            ...
            foreach (var tagged in PremierLeagueURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    ...
                    try
                    {
                        ...
                        this._logger.LogInformation("Checking available table IDs for {Team}", tagged.Label);
                        ...
                        if (goalkeepers != null && goalkeepers.Count == 0)
                        {
    ...
    public async Task<Result<List<URLInformation>>> CrawlEnglandChampionshipAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(CrawlEnglandChampionshipAsync)
        }))
        {
            _logger.LogInformation("Starting England Championship crawl");
            ...
            foreach (var tagged in EnglandChampionshipURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlTurkeySuperLigAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(CrawlTurkeySuperLigAsync)
        }))
        {
            _logger.LogInformation("Starting Turkey Super Lig crawl");
            ...
            foreach (var tagged in TurkeySuperLigURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
                    ...
                    try
                    {
                        ...
                        // Extract and import players
                        // var playersSelector = $"//div[@id='div_stats_standard_{tableIds.StandardStats}']";
                        var playersSelector = $"//table[@id='stats_standard_{tableIds.StandardStats}']";
                        _logger.LogInformation("Using table ID: {TableId} for team {Team}", tableIds.StandardStats, tagged.Label);
                        _logger.LogInformation("Player selector: {Selector}", playersSelector);
                        ...
                        if (goalkeepers != null && goalkeepers.Any())
                        {
                            ...
                            {
                                goalkeeper.Season = seasonUrl.Season;
                                goalkeeper.PlayerRefId = GenerateNumericPlayerRefId(goalkeeper.PlayerName, tagged.Label);
                            }
                        ...
                        if (shootings != null && shootings.Any())
                        {
                            ...
                            {
                                shooting.Season = seasonUrl.Season;
                                shooting.PlayerRefId = GenerateNumericPlayerRefId(shooting.PlayerName, tagged.Label);
                            }
            ...
            _logger.LogInformation("🎉 Completed Turkey Super Lig crawl with {Count} results", results.Count);
            return Result<List<URLInformation>>.Ok(results);
        }
    }

    public async Task<Result<List<URLInformation>>> CrawlChineseSuperLeagueAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
        ...
        {
            _logger.LogInformation("Starting Chinese Super League crawl");
            ...
            foreach (var tagged in ChineseSuperLeagueURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlGermanyBundeslia1Async()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
        ...
        {
            _logger.LogInformation("Starting Germany Bundeslia 1 crawl");
            ...
            foreach (var tagged in GermanyBundesliga1URLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlItalySerieAAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
        ...
        {
            _logger.LogInformation("Starting Italy Serie A crawl");
            ...
            foreach (var tagged in ItalySerieAURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlFranceLigue1Async()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(this.CrawlFranceLigue1Async)
        }))
        {
            _logger.LogInformation("Starting France Ligue 1 crawl");
            ...
            foreach (var tagged in FranceLigue1URLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlFranceLigue2Async()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(this.CrawlFranceLigue2Async)
        }))
        {
            _logger.LogInformation("Starting France Ligue 2 crawl");
            ...
            foreach (var tagged in FranceLigue2URLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlSpainLaligaAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
        ...
        {
            _logger.LogInformation("Starting Spain Laliga 1 crawl");
            ...
            foreach (var tagged in SpainLaliga1URLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlNetherlandsEredivisieAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(this.CrawlNetherlandsEredivisieAsync)
        }))
        {
            _logger.LogInformation("Starting Netherlands Eredivisie crawl");
            ...
            foreach (var tagged in NetherlandsEredivisieURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlAustraliaALeagueAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(this.CrawlAustraliaALeagueAsync)
        }))
        {
            _logger.LogInformation("Starting Australia A League crawl");
            ...
            foreach (var tagged in AustraliaALeagueURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlSwedenAllsvenskanAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(this.CrawlSwedenAllsvenskanAsync)
        }))
        {
            _logger.LogInformation("Starting Sweden Allsvenskan crawl");
            ...
            foreach (var tagged in SwedenAllsvenskanURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlArgentinaTorneoBetanoAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(this.CrawlArgentinaTorneoBetanoAsync)
        }))
        {
            _logger.LogInformation("Starting Argentina crawl");
            ...
            foreach (var tagged in ArgentinaTorneoBetanoURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlUSAMLSAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(this.CrawlUSAMLSAsync)
        }))
        {
            _logger.LogInformation("Starting USA MLS crawl");
            ...
            foreach (var tagged in USAMLSURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlBrazilSerieABetanoAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(this.CrawlBrazilSerieABetanoAsync)
        }))
        {
            _logger.LogInformation("Starting Brazil Serie A Betano crawl");
            ...
            foreach (var tagged in BrazilSerieABetanoURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlEcuadorLigaProAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(this.CrawlEcuadorLigaProAsync)
        }))
        {
            _logger.LogInformation("Starting Ecuador Liga Pro crawl");
            ...
            foreach (var tagged in EcuadorLigaProURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlColombiaPrimeraAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(this.CrawlColombiaPrimeraAsync)
        }))
        {
            _logger.LogInformation("Starting Colombia Primera crawl");
            ...
            foreach (var tagged in ColombiPrimeraAURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlChileLigaDePrimeraAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(this.CrawlChileLigaDePrimeraAsync)
        }))
        {
            _logger.LogInformation("Starting Chile De Primera crawl");
            ...
            foreach (var tagged in ChileLigaDePrimeraURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlSaudiArabiaProfessionalLeagueAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(CrawlSaudiArabiaProfessionalLeagueAsync)
        }))
        {
            _logger.LogInformation("Starting Saudi Arabia Professional League crawl");
            ...
            foreach (var tagged in SaudiArabiaProfessionalLeagueURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlMexicoLigaMXAsync()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(this.CrawlMexicoLigaMXAsync)
        }))
        {
            _logger.LogInformation("Starting Mexico Liga MX crawl");
            ...
            foreach (var tagged in MexicoLigaMXURLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
    ...
    public async Task<Result<List<URLInformation>>> CrawlKoreaKLeague1Async()
    {
        string correlationId = Guid.NewGuid().ToString();
        using (_logger.BeginScope(new Dictionary<string, object>
        {
            ["CorrelationId"] = correlationId,
            ["Operation"] = nameof(this.CrawlKoreaKLeague1Async)
        }))
        {
            _logger.LogInformation("Starting Korea K League 1 crawl");
            ...
            foreach (var tagged in KoreaKLeague1URLS.Urls)
            {
                foreach (var seasonUrl in tagged.SeasonUrls)
                {
                    var status = new URLInformation
